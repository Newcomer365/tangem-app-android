...
internal class LocalTogglesStorageTest {
    ...
    private val jsonAdapter = mockk<JsonAdapter<List<Toggle>>>()
    ...
    fun `failure initialize storage if assetReader throws exception`() = runTest {
        ...
        Truth.assertThat(storage.toggles).containsExactlyElementsIn(emptyList<Toggle>())
    }
    ...
    private fun everyCreatingMoshiAdapter() = every {
        val types = Types.newParameterizedType(List::class.java, Toggle::class.java)
        moshi.adapter<List<Toggle>>(types)
    }
